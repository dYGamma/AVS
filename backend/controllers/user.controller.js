...
class UserController {
        ...
        try {
            ...
            try {
                ...
                if (authHeader && typeof authHeader === 'string') {
                    ...
                    const currentUser = TokenService.validateAccessToken(token);
                    if (currentUser) {
                        isFriend = user.friends.some(f => f._id.toString() === currentUser.id);
                        requestSent = user.friendRequestsReceived.some(id => id.toString() === currentUser.id);
                        requestReceived = user.friendRequestsSent.some(id => id.toString() === currentUser.id);
                    }
                }
            } catch (e) {
            ...
            return res.json({ ...dto, isFriend, requestSent, requestReceived });
        } catch (e) {
    ...
    async requestFriend(req, res, next) {
        try {
            ...
            console.error('sendFriendRequest error:', e?.message || e);
            next(e);
        }
    }
    ...
    async acceptFriend(req, res, next) {
        try {
            ...
            if (!userId) return next(ApiError.UnauthorizedError());
            if (!requesterId) return next(ApiError.BadRequest('Не указан id отправителя'));

            await UserService.acceptFriendRequest(userId, requesterId);
            return res.json({ ok: true });
        } catch (e) {
            console.error('acceptFriendRequest error:', e?.message || e);
            next(e);
        }
    }
    ...
    async rejectFriendRequest(req, res, next) {
        try {
            ...
            console.error('rejectFriendRequest error:', e?.message || e);
            next(e);
        }
    }
    ...
    async getNotifications(req, res, next) {
        try {
            ...
            const notifications = await NotificationModel.find({ user: userId }).sort({ createdAt: -1 }).populate('from', 'nickname avatar_url');
    ...
    async markNotificationsRead(req, res, next) {
        try {
            ...
            console.error('markNotificationsRead error:', e?.message || e);
            next(e);
        }
    }
}

...
class UserService {
    ...
    async sendFriendRequest(fromUserId, toUserId) {
        ...
        if (fromUser.friends.includes(toUserId) || toUser.friends.includes(fromUserId)) {
            throw ApiError.BadRequest('Пользователи уже являются друзьями');
        }

        if (fromUser.friendRequestsSent.includes(toUserId) || toUser.friendRequestsReceived.includes(fromUserId)) {
            throw ApiError.BadRequest('Запрос в друзья уже отправлен');
        }
        ...
        await NotificationModel.create({
            ...
            type: 'friend_request',
            from: fromUserId,
            message: `${fromUser.nickname || fromUser.email} отправил вам запрос в друзья.`
        });
    ...
    async acceptFriendRequest(userId, requesterId) {
        ...
        if (!user || !requester) throw ApiError.BadRequest('Пользователь не найден');
    ...
    async removeFriend(userId, friendId) {
        ...
        if (!user || !friend) throw ApiError.BadRequest('Пользователь не найден');

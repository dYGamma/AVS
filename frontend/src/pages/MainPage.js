...
const HomeTabs = ({ activeTab, setActiveTab }) => (
  <div className="flex space-x-4 border-b border-theme mb-4 overflow-x-auto">
    {Object.entries(TABS_CONFIG).map(([key, { label }]) => (
      <button
        ...
        className={`pb-2 text-sm font-medium transition-colors whitespace-nowrap ${
          activeTab === key
            ? 'border-b-2 border-brand-purple text-theme'
 ...
 */
const BetaBlock = () => (
  <div className="bg-theme rounded-lg p-4 mb-4">
    <div className="flex items-start justify-between">
      <div>
        <h3 className="font-bold text-theme">Версия 1.0 в разработке</h3>
...
const MainPage = () => {
  ...
  return (
    <div className="p-4 bg-body text-theme">
      {/* Шапка */}
      <header className="flex items-center justify-between mb-4">
        <div className="relative flex-grow">
          <SearchIcon className="w-5 h-5 text-muted-theme absolute left-4 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            placeholder="Поиск аниме"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-theme border border-theme rounded-full py-2.5 pl-11 pr-4 text-theme placeholder-muted-theme focus:outline-none focus:ring-2 focus:ring-brand-purple"
          />
        </div>
        <div className="flex items-center ml-2">
          <button className="p-2 rounded-full hover-theme"><CogIcon className="w-6 h-6 text-muted-theme" /></button>
          <button className="p-2 rounded-full hover-theme"><BellIcon className="w-6 h-6 text-muted-theme" /></button>
        </div>
      </header>
      ...
      {/* Подсказка про поиск */}
      {debouncedSearch ? (
        <p className="text-sm text-muted-theme mb-3">Результаты поиска по «{debouncedSearch}»</p>
      ) : null}
      ...
      {!loading && animeList.length === 0 && !error && (
        <p className="text-center text-muted-theme mt-8">Ничего не найдено.</p>
      )}
    </div>
  );
};
